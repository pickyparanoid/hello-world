...
class Extended_queue {
 public:
  ...
  void clear();
  Error_code serve_and_retrieve(Plane&item);
...
class Runway
{
 ...
 Error_code Runway::can_land(const Plane& current)//判断是否可以降落
 {

 }

Error_code Runway::can_depart(const Plane& current)//判断是否可以起飞
{

}

Runway_activity activity(int time, Plane &moving)//判断跑道状态
{
